...
**Repair status (2026-09-17, baseline `6979d528b`).** The rows below stay as the record of what the audit found. R1 has since closed the priority and acceptance half of F1, all of F3, F2, and the reconciliation half of F4: a confirmed lane now keeps the priority it declared in its own Todo label, the settlement receipt retains each lane's acceptance beside the Todo identity it became, a partial application reports `team_plan_partially_applied` with a gap count instead of a full success, a confirmation that staffed no lane is recorded as the typed failure `team_plan_no_staffable_lane` rather than as an applied plan with an empty Todo id, a team-plan preview now binds this Goal's active-state intent and the canonical basis its lanes would advance, and a lane write that fails after earlier lanes were committed is recorded as the typed failure `team_plan_lane_write_failed` whose details name the lanes that exist and the lane that failed, so the retry reconciles against real identities instead of duplicating them. The remainder is open and owned by canonical Todos: the plan-level quota/stop classification, F4's automatic recovery and execution barrier (the re-entrant apply that finishes a partial plan without a fresh owner confirmation), and rendering a partial application as partial on the confirmation card.
